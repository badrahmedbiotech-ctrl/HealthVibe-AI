...
print("🔄 1. جاري تحميل الداتا سيت الطبية المعتمدة...")
df = pd.read_csv("dataset/hypertension_dataset.csv")
df.columns = df.columns.str.strip()

print("🧬 2. تطبيق هندسة الميزات (Feature Engineering) لرفع الدقة...")
# مؤشرات طبية حقيقية مشتقة من قراءات الضغط والعمر والوزن
df['pulse_pressure'] = df['sysBP'] - df['diaBP']                          # فرق الضغط الانقباضي والانبساطي
df['mean_arterial_pressure'] = df['diaBP'] + (df['pulse_pressure'] / 3)   # متوسط ضغط الشريان
df['age_bmi'] = df['age'] * df['BMI']                                     # تفاعل العمر مع كتلة الجسم
df['smoking_load'] = df['currentSmoker'] * df['cigsPerDay'].fillna(0)     # حمل التدخين الفعلي
